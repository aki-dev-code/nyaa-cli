...
def search_torrent(
    ...
    with Progress(
        ...
        for entry in feed.entries:
            ...
            if episode:
                if title.get("type") != "episode":
                    logger.debug(
                        f"Skipping (Not episode, but searching for E{episode}) {entry}"
                    )
                    continue

                if title.get("episode") != episode:
                    logger.debug(
                        f"Skipping (Wrong episode, searching for E{episode}) {entry}"
                    )
                    continue
        ...
        if not entries:
            print(
                f"[bold red]No results found for search: '{search_query.replace('%20', ' ')}'[/bold red]"
            )
            return None
        ...
        choices = [
            {"name": entry.display_title, "value": index}
            for index, entry in enumerate(entries[:number])
        ]
    ...
    logger.debug(
        f"Downloading torrent file to '{torrent_path}' from '{entry_choice.link}'"
    )
